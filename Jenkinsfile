...
pipeline {
    ...
    stages {
        ...
        stage('Tag & Push Frontend Image to ECR') {
            steps {
                sh '''
                docker tag frontend-image:latest ${AWS_ACCOUNT_ID}.dkr.ecr.${AWS_REGION}.amazonaws.com/${FRONTEND_REPO}:latest
                docker push ${AWS_ACCOUNT_ID}.dkr.ecr.${AWS_REGION}.amazonaws.com/${FRONTEND_REPO}:latest
                '''
            }
        }
        ...
        stage('Tag & Push MySQL Image to ECR') {
            steps {
                sh '''
                docker tag mysql:latest ${AWS_ACCOUNT_ID}.dkr.ecr.${AWS_REGION}.amazonaws.com/${MYSQL_REPO}:latest
                docker push ${AWS_ACCOUNT_ID}.dkr.ecr.${AWS_REGION}.amazonaws.com/${MYSQL_REPO}:latest
                '''
            }
        }
        ...
        stage('Deploy to Server') {
            steps {
                sshagent(['ec2-ssh-key']) {
                    sh '''
                    ssh -o StrictHostKeyChecking=no ec2-user@your-server-ip << 'EOF'
                    aws ecr get-login-password --region $AWS_REGION | docker login --username AWS --password-stdin ${AWS_ACCOUNT_ID}.dkr.ecr.${AWS_REGION}.amazonaws.com
